...
class _SearchPageState extends State<SearchPage> {
  ...
  Future<void> getWebsiteData() async {
    final url = Uri.parse('https://parcelsapp.com/en/tracking/RR002550306BD');
    final response = await http.get(url);
    dom.Document document = parser.parse(response.body);

    document.body;
    final titles = document.querySelectorAll(
        '#tracking-info > div:nth-child(1) > div.row.parcel > div.col-md-8.col-lg-8 > ul > li:nth-child(1) > div.event-time').map((element) =>
        element.innerHtml.trim()).toList();

    final urls = document.querySelectorAll('h2 > a').map((element) => 'https://www.amazon.com/${element.attributes['href']}').toList();

    final urlImage = document.querySelectorAll('span > a > div > img').map((element) => element.attributes['src']!).toList();

    setState(() {
      articles = List.generate(titles.length, (index) => Article(title: titles[index], url: urls[index], urlImage: urlImage[index],),);
    });
  }
   ...
   .runJavaScriptReturningResult(javaScript)
    ...
    ..setNavigationDelegate(NavigationDelegate(
      ...
      onPageFinished: (String url) async {
        // final html = await webViewController.runJavaScriptReturningResult(javaScript)
      },
    ...
    ..loadRequest(Uri.parse('https://parcelsapp.com/en/tracking/RR002550306BD'));

  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Web Scraping')),
      body: ListView.builder(itemCount: articles.length, padding: const EdgeInsets.all(12), itemBuilder: (context, index) {
        final article = articles[index];

        return ListTile(leading: Image.network(article.urlImage, width: 50, fit: BoxFit.fitHeight), title: Text(article.title), subtitle: Text(article.url),);
      },),
      floatingActionButton: Offstage(offstage: false, child: WebViewWidget(controller: webViewController)),);
  }
}
